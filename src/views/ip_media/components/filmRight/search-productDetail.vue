  ...
  <div>
    <el-form class="searchBar" :inline="true" :model="formInline"
      ...
      <el-form-item label="产品状态">
        <SelectItem
          type="rightP_item_status"
          v-model="formInline.item_status"
          width="140px"
        />
      </el-form-item>
      ...
      <el-form-item>
        ...
        <el-button type="primary"
                   native-type="submit" :disabled="exporting"
                   @click="handleQuery('export')"
                   v-if="$checkPermission(['edit'])">导出
          <i v-if="exporting" class="el-icon-loading"></i>
        </el-button>
        <el-button type="primary"
                   native-type="submit" :disabled="uploadObj.show"
                   @click="handleUpload()"
                   v-if="$checkPermission(['edit'])">导入
...
import {getExcel} from "@/vendor/Export2Zip";
...
const formInline = {
  ...
  user: null,
  department: null,
  authorized_area: null,
  adds: null,
  term: null,
  item_status: null,
};
export default {
  name: 'product-rightDetail',
  components: {
    SelectItem,
    SelectItmeRemote,
    uploadRight,
  ...
  methods: {
    async handleQuery(str) {
      ...
      body.item_status = this.formInline.item_status && this.formInline.item_status.value;
    ...
    async exportData(params) {
      ...
      // console.log({ ...params})
      const res = await this.$store.dispatch(
        "property/exportDetail",
        { ...params}
      );
      if (!res.code) {
        const downloadFilename = 'product_copyright_details_' + new Date().toLocaleDateString().replace('/','_') + ".csv";//设置导出的文件名
        getExcel(res, downloadFilename)
