<template>
  <div class="app-container">
    ...
    <el-dialog
      :title="
        dialogType == 'new' ? '新增' : dialogType == 'edit' ? '修改' : '选择'
      "
...
export default {
  ...
  methods: {
    getData(data) {
      ...
      this.propertyListPost();
    },

    async propertyListPost() {
      const params = {
    ...
    async propertyList() {
      this.tableLoading = true;
      const res = await this.$store.dispatch("property/propertyList", {
        page: this.listQuery.page,
        // page_size: this.listQuery.limit,
      });
    ...
    async handleCommand(command) {
      ...
      this.dialogType = "new";
      this.activeName = "REGION";
      this.basicsForm = JSON.parse(JSON.stringify(basicsForm));
    },
    async handelEdit(row) {
      this.$set(row, "loading", true);
      const res = await this.$store.dispatch("property/propertyDetail", {
        id: row._id,
      });
      if (res.code === 0) {
    ...
    handleSave() {
      ...
      if (this.dialogType === "new") {
        return this.property_add();
      }

      this.propertyEdit();
      this.propertyMainEdit();
    },
    async propertyMainEdit() {
      ...
      const res = await this.$store.dispatch("property/propertyMainEdit", body);
      if (res.code === 0) {
    ...
    async property_add() {
      ...
      const res = await this.$store.dispatch("property/property_add", body);
      if (res.code === 0) {
        this.dialogVisible = false;
