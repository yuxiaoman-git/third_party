...
export function getIds(list, partition = ',') {
  if (!list) {
    return ''
  }
  if (Array.isArray(list)) {
    return list.map((el) => el.value).join(partition)
  }
  return ''
}
...
export function isObjectValueEqual(obj1, obj2) {
  ...
  for (var i = 0; i < aProps.length; i++) {
    ...
    // 故先判断两边都有相同键名
    if (!Object.prototype.hasOwnProperty.call(obj2, propName)) {
      return false
    }
...
export const validate = function (rule, value, callback) {
